...
trees.each do |target|
	if target.spp.include?(targetspp)&&target.dbh01!=0.0&&target.x<=xmax&&target.x>=xmin&&target.y<=ymax&&target.y>=ymin
...
CSV.open('dctr0823bp.csv','w') do |test|
	for i in 0..kazu do
		test << [Num[i], Xx[i],Yy[i],Spp[i],Dbh01[i],Dbh04[i],Hgt[i],Crd1[i],Crd2[i],Crd3[i],Crd4[i],Crd5[i],Crd6[i],Crd7[i],Crd8[i],Crd9[i],Kabudachi[i],Death[i]]
